...
def run_pipeline(topic: str) -> dict:
    load_default_config()
    run_id = now_run_id(topic)
    rd = run_dir(run_id)

    payload = do_ingest(topic=topic, input_path=None, run_id=run_id)
    cm = do_extract(ingest_payload=payload, run_id=run_id)
    outline = do_plan(concept_map=cm, run_id=run_id)
    scene_plan = do_plan_scenes(concept_map=cm, lesson_outline=outline, run_id=run_id)
    do_codegen(scene_plan=scene_plan, run_id=run_id)
    do_study(concept_map=cm, lesson_outline=outline, run_id=run_id)
...
class handler(BaseHTTPRequestHandler):  # noqa: N801 — Vercel expects this name
    ...
    def do_GET(self) -> None:  # noqa: N802
        self._send(200, {
            ...
            "usage": "POST /api/generate with JSON body {\"topic\": \"...\"}",
        })
    ...
    def do_POST(self) -> None:  # noqa: N802
        try:
            ...
            if not topic:
                return self._send(400, {"ok": False, "error": "missing 'topic'"})
            if len(topic) > 200:
